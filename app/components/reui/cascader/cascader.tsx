...
function Cascader<T>({
  ...
  const levels = React.useMemo(() => {
    ...
    return [path.length ? path.at(-1) : CASCADER_ROOT_KEY];
  ...
  const currentParentValue = path.length ? path.at(-1) : null;
...
function CascaderEmpty({
  ...
  const levelKey = path.length ? path.at(-1) : CASCADER_ROOT_KEY;
    ...
    body = (
      <span data-slot="cascader-error" className="flex flex-col items-center gap-1.5">
        ...
        <button
          ...
          onClick={() => retryLevel(levelKey as string)}
 ...
 */
function CascaderStatus({ className, children, ...props }: CascaderStatusProps) {
