  ...
  useLayoutEffect(() => {
    ...
    const indicatorLeft =
      typeof columnResizing.startOffset === "number" && viewportElement
        ? columnResizing.startOffset - viewportElement.getBoundingClientRect().left
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="bg-primary rounded-b-lg absolute top-0 left-0 -translate-x-1/2 shadow-xs"
        style={{ width: 5 }}
      />
    </div>
  );
...
function DataGridTableBody({ children }: { children: ReactNode }) {
  ...
  return (
    <tbody
      ...
      className={cn(
        props.tableLayout?.rowRounded && "[&_td:first-child]:rounded-l-3xl",
        props.tableLayout?.rowRounded && "[&_td:last-child]:rounded-r-3xl",
...
function DataGridTableLoader() {
  ...
  return (
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
      <div className="text-muted-foreground bg-card rounded-3xl flex items-center gap-2 border px-4 py-2 text-sm leading-none font-medium">
