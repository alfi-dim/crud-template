...
import { FILTER_ROW_SELECTOR, useFilterRowDrag } from "~/components/reui/filters/filters-dnd";
...
type FilterColumn = (typeof CONTENT_COLUMNS)[number] | (typeof ACTION_COLUMNS)[number];
...
const FilterDragContext = React.createContext<ReturnType<typeof useFilterRowDrag> | null>(null);
...
const CELL_INVALID_CLASS = "data-invalid:ring-destructive/40 data-invalid:ring-1";
...
const ACTION_BAND_CLASS = cn("text-muted-foreground flex shrink-0 items-center gap-0.5 pe-1");
...
function ownCells(row: HTMLElement): HTMLElement[] {
  return Array.from(row.querySelectorAll<HTMLElement>(`[${CELL_ATTRIBUTE}]`)).filter(
    (cell) => cell.closest<HTMLElement>(ROW_SELECTOR) === row,
  );
}
...
function CombinatorCell({
  ...
  if (slot === "where") {
    return <span className={cn(COMBINATOR_TEXT_CLASS, className)}>{actions.labels.where}</span>;
  }
...
function canFilterNodeMove(query: FilterQuery<unknown>, position: { parentId: string }) {
...
function RowIssueHint({ message, className }: { message: string; className?: string }) {
...
function FilterInlineValueCell<V, O>({
  ...
  const committed = rule.value === undefined || rule.value === null ? "" : String(rule.value);
...
function FilterAdvancedRowImpl<V, O>({ rule, position }: FilterAdvancedRowProps<V>) {
  const actions = useFilterActions<V, O>();
  ...
  const [pickerPath, setPickerPath] = React.useState<string[]>(() => rule.path.slice(0, -1));
  ...
  const active = useActiveColumn(rule.id, columns, position.depth === 1 && position.index === 0);
  ...
  return (
    <div
      ...
      role="group"
      aria-label={actions.labels.rowLabel(`${pathText} ${operatorLabel}`, position.depth)}
    ...
    >
      ...
      <div className={CONTENT_BAND_CLASS}>
        <div className={cn(CELL_BOX_CLASS, FIELD_CELL_CLASS)}>
          ...
          >
            ...
            <PopoverContent
              align="start"
              className={cn(FILTER_FIELD_PICKER_CLASS, actions.fieldPickerClassName)}
            >
            ...
            <FilterOperatorPopover
              ...
              trigger={
                <Button
                  ...
                  title={operatorLabel}
                  className={cn(CELL_CLASS, CELL_INVALID_CLASS, "text-muted-foreground")}
                ...
                <FilterValuePopover
                  ...
                  trigger={
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1 truncate text-start">{valueLabel}</span>
      ...
      <div className={ACTION_BAND_CLASS}>
        {canMove ? <RowHandle nodeId={rule.id} active={active} onFocus={onCellFocus} /> : null}
        <DropdownMenu
          open={menuOpen}
          onOpenChange={(next) => {
            if (next && locked) return;
...
export const FilterAdvancedRow = React.memo(FilterAdvancedRowImpl) as typeof FilterAdvancedRowImpl;
...
function FilterUnknownRow<V>({ rule, position }: { rule: FilterRule<V>; position: RowPosition }) {
  const actions = useFilterActions();
  const sizes = filterControlSizes(actions);
  return (
    <div
...
function FilterAdvancedGroup<V, O>({
  ...
  const active = useActiveColumn(group.id, columns, position.depth === 1 && position.index === 0);
  ...
  const description =
    group.combinator === "and" ? actions.labels.groupAll : actions.labels.groupAny;
  ...
  return (
    ...
    >
      <div className={cn(COMBINATOR_CLASS, CELL_BOX_CLASS, GROUP_COMBINATOR_WRAP_CLASS)}>
        <CombinatorCell
          index={position.index}
          parentId={position.parentId}
          combinator={position.combinator}
      ...
      >
        ...
        <div className={cn(TRACK_CONTAINER_CLASS, "flex flex-col", FILTER_ROW_GAP_CLASS)}>
        ...
        >
          ...
          <div className={cn("ms-auto", ACTION_BAND_CLASS)}>
            {canMove ? <RowHandle nodeId={group.id} active={active} onFocus={onCellFocus} /> : null}
            <FilterGroupMenu group={group} active={active} onFocus={onCellFocus} />
          </div>
        </div>
      </div>
    </div>
...
function FilterGroupMenu<V>({
  ...
  return (
    ...
    >
      ...
      <DropdownMenuContent align="end" className={cn(FILTER_MENU_CLASS, actions.menuClassName)}>
        <DropdownMenuItem disabled={locked} onClick={() => actions.duplicateNode(group.id)}>
          <CopyIcon aria-hidden="true" />
          <span className={FILTER_MENU_LABEL_CLASS}>{actions.labels.duplicate}</span>
        </DropdownMenuItem>
        <DropdownMenuItem disabled={locked} onClick={() => actions.unwrapGroup(group.id)}>
          {/* The inverse of the row menu's layers glyph. */}
          <UngroupIcon aria-hidden="true" />
          <span className={FILTER_MENU_LABEL_CLASS}>{actions.labels.ungroup}</span>
        </DropdownMenuItem>
        ...
        >
          ...
          <span className={FILTER_MENU_LABEL_CLASS}>{actions.labels.removeGroup}</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
function addFilterRow<V, O>(
  ...
  const entry = actions.index.all.find((candidate) => isFilterFieldPickable(candidate.field));
...
export function FiltersAdvancedPanel<V, O>({
  ...
  const sizes = filterControlSizes(actions);
  const { query, ruleCount, announcement, announcementSeq } = useFilterState<V>();
  const focusStore = useFilterFocusStore();
  const rowStateStore = useFilterRowStateStore();
    ...
    () =>
      collectFilterIssues(
        ...
        (rule) => {
          const field = getFilterField(actions.index, rule.path);
          if (!field) return null;
          return getFilterArity(getFilterOperator(actions.resolveOperators(field), rule.operator));
        },
  ...
  const restoreFocus = React.useCallback(() => {
    ...
    const rows = Array.from(bodyRef.current?.querySelectorAll<HTMLElement>(ROW_SELECTOR) ?? []);
    const { id, segment } = focusStore.getSnapshot();
    const row = id ? rows.find((candidate) => candidate.dataset.nodeId === id) : undefined;
  ...
  const onKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    const cell = (event.target as HTMLElement).closest<HTMLElement>(`[${CELL_ATTRIBUTE}]`);
    ...
    for (const entry of body.querySelectorAll<HTMLElement>(`[${CELL_ATTRIBUTE}]`)) {
      ...
      renderEmptyState ? (
        ...
        >
          ...
          {actions.readOnly ? null : (
            <p className="text-muted-foreground text-xs">{actions.labels.builderEmptyHint}</p>
          )}
        </div>
      )}
  ...
  return (
    <div
      ...
      style={
        {
          [PANEL_PAD_VAR]: mode === "inline" ? PANEL_PAD_INLINE : PANEL_PAD_POPOVER,
...
export function FiltersAdvanced<V, O>({
  ...
  if (mode === "inline") {
    return (
      <FiltersAdvancedPanel<V, O> mode="inline" reorderable={reorderable} className={className} />
    );
  }
