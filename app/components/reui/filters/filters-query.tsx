 ...
 */
export type FilterArityResolver<V> = (rule: FilterRule<V>) => FilterOperator["arity"] | null;

/** Runs a field's own `validate`. A resolver like `arityOf`: no schema here. */
export type FilterValidateResolver<V> = (rule: FilterRule<V>) => string | null | undefined | false;
...
export function toggleFilterCombinator<V>(query: FilterQuery<V>, groupId: string): FilterQuery<V> {
 ...
 */
function detachFilterNode<V>(group: FilterGroupNode<V>, id: string): FilterGroupNode<V> {
  let changed = false;
  const rules: FilterNode<V>[] = [];
...
export function wrapFilterNodeInGroup<V>(
  ...
  return rewriteGroup(
    ...
    (group) => ({
      ...group,
      rules: group.rules.map((child) =>
        child.id === id ? createFilterGroup<V>({ id: groupId, combinator, rules: [child] }) : child,
      ),
    }),
  ) as FilterQuery<V>;
}
